...
public class anonceservice implements IService<Anonce> {
    ...
    public void ajouter(Anonce anonce) throws SQLException {
    ...
    public void modifier(Anonce anonce) throws SQLException {
    ...
    pstmt.setString(1, anonce.getTitre());
    pstmt.setString(2, anonce.getDescription());
    pstmt.setDate(3, new Date(anonce.getDateDisponibilite().getTime()));  // Conversion de Date en SQL Date
    pstmt.setDouble(4, anonce.getPrixParJour());
    pstmt.setInt(5, anonce.getVehiculeId());
    pstmt.setInt(6, anonce.getUtilisateurId());
    pstmt.setString(7, anonce.getPhotoVehicule());
  ...
  public void supprimer(Anonce anonce) throws SQLException {
    ...
    public List<Anonce> recuperer() throws SQLException {
    ...
    public List<Anonce> rechercherParMotCleEtConducteur(String motCle, int conducteurId) throws SQLException {
        List<Anonce> resultats = new ArrayList<>();
        String requete = "SELECT * FROM anonces WHERE utilisateur_id = ? AND " +
                "(titre_a LIKE ? OR description_a LIKE ? OR adresse LIKE ?)";
        ...
        try (PreparedStatement ps = con.prepareStatement(requete)) {
            ...
            while (rs.next()) {
                ...
                Anonce anonce = new Anonce(id_anonce, titre, description, dateDisponibilite, prixj,
                        vehiculeId, conducteurId, photoVehicule, adresse);
                anonce.setLatitude(latitude);
                anonce.setLongitude(longitude);
