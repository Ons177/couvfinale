...
public class PostDetailController {
    ...
    private int current; // Will store the current user's ID
    private final String DEFAULT_IMAGE_PATH = "/images/default.png";
    ...
    public void setPost(Post post) {
        ...
        this.postownerid = post.getUser_id();
...
//user = finduserbyid(); , name = user.getname();
        // You could add a real username here if available
        UserService serviceuser = new UserService();
        String nom = serviceuser.getUserById(comment.getUser_id()).getNom();
        ...
        if (comment.getUser_id() == currentUser.getId_utilisateur()) {
            // Create action buttons container
            HBox actionButtons = new HBox(10);
            actionButtons.setAlignment(Pos.CENTER_RIGHT);

            // Create delete button (circular with trash icon)
            Button deleteButton = createCircularButton("/images/delete.png", "delete-button");
            deleteButton.setTooltip(new Tooltip("Supprimer"));

            // Create edit button (circular with edit icon)
            Button editButton = createCircularButton("/images/edit.png", "edit-button");
            editButton.setTooltip(new Tooltip("Modifier"));

            // Add action handlers
            deleteButton.setOnAction(e -> handleDeleteComment(comment, commentBox));
            editButton.setOnAction(e -> handleEditComment(comment, contentLabel, commentBox));
    ...
    private void handleSendComment() {
        ...
        if (commentText.isEmpty()) {
            showAlert("⚠️ Veuillez écrire un commentaire avant d'envoyer.");
            return;
        }

        try {
            // Étape 1 : Vérification des propos inappropriés via l'API
            if (FrenchProfanityAPI.containsProfanity(commentText)) {
                showAlert("⚠️ Votre commentaire contient des mots inappropriés. Veuillez les modifier.");
                return;
            }

            // Étape 2 : Si aucun problème, créer le commentaire
            Commentaire newComment = new Commentaire();
            newComment.setComment_contenu(commentText);
            newComment.setPost_id(post.getPost_id());
            newComment.setUser_id(currentUser.getId_utilisateur());
            newComment.setDate_commentaire(LocalDateTime.now());

            serviceCommentaire.ajouter(newComment);

            // Étape 3 : Rafraîchir les commentaires
            loadComments();

            // Étape 4 : Réinitialiser le champ
            commentArea.clear();

        } catch (Exception e) {
            e.printStackTrace();
            showAlert("❌ Une erreur est survenue lors de l'envoi du commentaire : " + e.getMessage());
        }
    }
