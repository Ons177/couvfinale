...
public class AnonceAcceuilcontrolleur implements Initializable {
    ...
    private void afficherAnonces() {
        ...
        try {
            List<Anonce> annonces = service.recuperer();
    ...
    private void afficherMesAnonces() {
        ...
        try {
            if ("admin".equalsIgnoreCase(currentUser.getRole())) {
                ...
                List<Anonce> mesAnonces = service.recuperer();
    ...
    private void rechercherEtAfficherAnnonces(String motCle) {
        ...
        try {
            ...
            if ("conducteur".equals(role)) {
                if (motCle.isEmpty()) {
                    resultats = service.getAnoncesParConducteur(currentUser.getId_utilisateur());
                } else {
                    resultats = service.rechercherParMotCleEtConducteur(motCle, currentUser.getId_utilisateur());
                }
                afficherAnoncesDansFlowPane(resultats, "/Useranonceitem.fxml");
            } else {
                if (motCle.isEmpty()) {
                    resultats = service.recuperer(); // toutes les annonces pour passager
                } else {
    ...
    @FXML
    public void deconn(ActionEvent actionEvent) {
        try {
    ...
    public void retourmenu(ActionEvent actionEvent) {
        try {
            ...
            String role = UserSession.getCurrentUser().getRole(); // Exemple : "admin", "conducteur", "passager"

            String fxmlPath = null;

            switch (role.toLowerCase()) {
                case "admin":
                    fxmlPath = "/Views/MenuAdmin.fxml";
                    break;
                case "conducteur":
